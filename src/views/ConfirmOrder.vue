    ...
    <div class="confirmOrder">
      ...
      <div class="main">
            <div class="info">
                ...
                <div class="choose">
                    <h3>支付方式 <span class="pay">{{description}}</span></h3>
                    <div class="choosebg">
                        <span @click="selectPayment(item)" v-for="item in payModes" :key="item.code">{{item.description}}</span>
                    </div>
                </div>
                <ul class="foot">
...
// api
import {settlement,alipay,wxpay,queryAlipay,queryWxpay} from '@/utils/api/order'
...
const selectPayment = (item)=>{
    ...
    endPayModes.value = item.code
    if(endPayModes.value == "alipayment"){
        alipay({
            courses:cartStore.orderList,
            payModes:endPayModes.value
        }).then(res=>{
    ...
    }else if(endPayModes.value == "wxpayment"){
        wxpay({
            courses:cartStore.orderList,
            payModes:endPayModes.value
        }).then(res=>{
...
const goPayment = ()=>{
    // 判断是否选择了支付方式
    if(endPayModes.value == ''){
        ElMessage.warning({
            message: '请先选择支付方式!'
        });
        return;
    }else if( endPayModes.value =='alipayment' ){
